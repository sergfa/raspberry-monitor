...
def start_presence_monitor(timeout, beaconQueue):
    
    from threading import Thread
    global presence_data
    presence_data = loadInitialPresences()
    
    monitor_device_state_thread = Thread(name='monitor_device_state', target=monitor_device_state, kwargs={"presence_data" : presence_data, "timeout":timeout, "beaconQueue": beaconQueue})
